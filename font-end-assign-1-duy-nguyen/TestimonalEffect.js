let allComments = document.getElementsByClassName("comment-content-wrap");
let allCommentsOrder = [1, 2, 3];
let allCurrentPositions = [0, 0, 0];
let allMoveDistanes = [];
let moveCmtBtn = document.getElementsByClassName("move-cmt-btn");
let selcCmtBtn = document.getElementsByClassName("select-specific-cmt-btn");
...
function changeOrder(direction, thisBtn) {
    if (direction === "left") {
        ...
        let selctedCmtBtn = allCommentsOrder[Math.ceil(allCommentsOrder.length / 2) - 1] - 1;
        Array.from(selcCmtBtn).forEach((each, index) =>index != selctedCmtBtn?(each.style.background = "rgb(212, 224, 212)"): (each.style.background = "green"));
        return;
    }
    if (direction === "right") {
    ...
    function handleSelectCmt(newOrder, thisBtn) {
        ...
        Array.from(selcCmtBtn).forEach(each => each.style.background = "rgb(212, 224, 212)");
...
function updatePos() {
    ...
    Array.from(allComments).forEach((each, index) =>allCurrentPositions.push(each.offsetLeft));
}
...
function handleMoveComment(moveDistance, direction) {
    Array.from(moveCmtBtn).forEach((each, index) => each.style.pointerEvents = "none");
    Array.from(allComments).forEach((each, index) => {
    ...
    this.tempTimer = window.setInterval(() => {
        step --;
        Array.from(allComments).forEach((each, index) => each.style.left = (-moveDistance[index] * step) / 30 + "px");
        if (step === 0) {
            clearInterval(this.tempTimer);
            Array.from(allComments).forEach((each, index) => each.style.opacity = "1");
            Array.from(moveCmtBtn).forEach((each, index) => each.style.pointerEvents = "auto");
        }
    }, 8);
}
